function net=myinitw(net,i)
%  NITNW Nguyen-Widrow layer initialization function.
%
%  Syntax
%
%    net = initnw(net,i)
...
for j=1:numInputs
  if size(net.inputs{inputInds(j)}.userdata, 1) >= i
    % use user-specified input weights, instead of the NW ones
    net.IW{i,inputInds(j)} = net.inputs{inputInds(j)}.userdata(i,:);
  else
    % use default NW weights
